 ...
 */
 ...
 class DBIP {
  ...
  const TYPE_COUNTRY_V3 = 3;
  const TYPE_LOCATION_V3 = 4;
  const TYPE_ISP_V3 = 5;
  const TYPE_FULL_V3 = 6;

  const TYPE_LOCATION_V2 = 7;
  const TYPE_ISP_V2 = 8;
  const TYPE_FULL_V2 = 9;

  const TYPE_COUNTRY = self::TYPE_COUNTRY_V3;
  const TYPE_LOCATION = self::TYPE_LOCATION_V3;
  const TYPE_ISP = self::TYPE_ISP_V3;
  const TYPE_FULL = self::TYPE_FULL_V3;
  ...
  protected function prepareImport($tableName, array $fields) {
 	 try {
 		 if ($this->db->query("select count(*) as cnt from `{$tableName}`")->fetchObject()->cnt) {
 			 throw new DBIPException("table {$tableName} is not empty");
 		 }
 		 $this->db->setAttribute(\PDO::ATTR_EMULATE_PREPARES, false);
 		 $this->db->beginTransaction();
 		 $q = $this->db->prepare("insert into `{$tableName}` (" . implode(",", $fields) . ",addr_type) values (" . implode(",", array_fill(0, count($fields), "?")) . ",?)");
 		 return $q;
  ...
  protected function doLookup($tableName, $addrType, $addrStart) {
 	 $q = $this->db->prepare("select * from `{$tableName}` where addr_type = ? and ip_start <= ? order by ip_start desc limit 1");
 ...
 */
 class DBIPMySQL extends DBIP {
  ...
  protected function prepareImport($tableName, array $fields) {
 	 $q = mysql_query("select count(*) as cnt from `{$tableName}`", $this->db);
  ...
  protected function doLookup($tableName, $addrType, $addrStart) {
 	 $q = mysql_query("select * from `{$tableName}` where addr_type = '{$addrType}' and ip_start <= '" . mysql_real_escape_string($addrStart) . "' order by ip_start desc limit 1", $this->db);
 	 $r = mysql_fetch_object($q);
 	 mysql_free_result($q);
 	 return $r;
  }
 ...
 */
 class DBIPMySQLI extends DBIP {
  ...
  protected function prepareImport($tableName, array $fields) {
 	 $q = mysqli_query($this->db, "select count(*) as cnt from `{$tableName}`");
  ...
  protected function doLookup($tableName, $addrType, $addrStart) {
 	 $q = mysqli_query($this->db, "select * from `{$tableName}` where addr_type = '{$addrType}' and ip_start <= '" . mysqli_real_escape_string($this->db, $addrStart) . "' order by ip_start desc limit 1");
 	 $r = mysqli_fetch_object($q);
 	 mysqli_free_result($q);
 	 return $r;
  }
