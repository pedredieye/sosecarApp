...
class Helper
{
    ...
    public function checkPrivillieges($user_role_id, $autorized_id = [1,2])
    {
      return ( in_array($user_role_id,$autorized_id));
    }
    ...
    public static function checkConnexion ()
    {
      if(isset($_COOKIE['usr_id'])){
        foreach ($_COOKIE as $key => $value) {
          setcookie($key, $value, time() + 3600, "/admin9763/");
        }
        return 1;
      }
      else
    ...
    public function setCookie($data)
    {
       foreach ($data as $key => $value) {
         setcookie($key, $value, time() + 48*3600, "/admin9763/");
     ...
     public function getUsr($container)
     {
       if(isset($_COOKIE['usr_id'])){
         $id = $_COOKIE['usr_id'];

         $usr = User::where('id',self::decodeData($container, $id))->with('hospital')->with('role')->first();
         if($usr)
           return $usr->toArray();
    ...
    public function curl_get_fields($url, $fields, $header=false){
         $get_array = [];
         if ( is_array($fields) ) {
        ...
        $options=array(
              ...
              CURLOPT_RETURNTRANSFER => false,    // Retourner le contenu téléchargé dans une chaine (au lieu de l'afficher directement)
    ...
    public static function qrCodeOld($data, $file_name, $text = 'CARDIOTECH / ASCAOC')
    {
      $qrCode = new QrCode();
      ...
      $qrCode
          ...
          ->setForegroundColor(array('r' => 66, 'g' => 133, 'b' => 244, 'a' => 0))
    ...
    public static function getParticipant($container)
    {
      if(isset($_COOKIE['usr_id'])){
        $id = $_COOKIE['usr_id'];
    ...
    public static function getParticipantFromRef($ref)
    {
